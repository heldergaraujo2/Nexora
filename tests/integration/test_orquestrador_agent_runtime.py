...
class _RoutedProvider(Provider):
    def __init__(self, modelo: str, *, tokens: bool = False) -> None:
    ...
    def generate(self, _prompt: str, **_kwargs) -> GenerationResult:
        self.chamadas += 1
        usage = {"prompt_tokens": 9, "completion_tokens": 6, "total_tokens": 15} if self.tokens else {}
        return GenerationResult(text=f"resultado:{self.modelo}", tool_calls=[], usage=usage)


def test_orquestrador_delega_ciclo_de_provider_ao_runtime() -> None:
    provider = _SequenceProvider(["", "resultado valido"])
    orquestrador = Orquestrador(rotador=_Rotador(provider), provider=provider, planejador=lambda _: [{"id": "t1", "descricao": "gerar resultado"}])
    ...
    assert any(evento["tipo"] == "observacao" for evento in orquestrador.historico)
...
def test_orquestrador_nao_deixa_excecao_de_provider_escapar() -> None:
    ...
    orquestrador = Orquestrador(rotador=_Rotador(provider), provider=provider, planejador=lambda _: [{"id": "t1", "descricao": "executar"}], max_tentativas=3)
...
def test_orquestrador_ferramenta_passa_uma_vez_pela_governanca() -> None:
    ...
    policy = PolicyEngine(regras=(RegraPolitica(id="allow-orchestrator-tool", efeito=EfeitoPolitica.ALLOW, solicitante="orchestrator", executor="ferramenta_teste", tarefa="executar"),), origem="integration-test")
    ...
    registry.registrar(Ferramenta(nome="ferramenta_teste", descricao="Ferramenta de teste controlada", executar=executar_ferramenta))
    provider = _SequenceProvider(["provider-nao-deve-ser-chamado"])
    orquestrador = Orquestrador(rotador=_Rotador(provider), provider=provider, ferramentas=registry, planejador=lambda _: [{"id": "t1", "descricao": "executar ferramenta", "ferramenta": "ferramenta_teste", "parametros": {"valor": "42"}}])
...
def test_orquestrador_rota_provider_e_modelo_realmente_e_registra_decisao_no_trace() -> None:
    ...
    hardware = PerfilHardware(sistema="Windows", arquitetura="AMD64", cpu_nucleos=6, ram_bytes=16 * 1024**3)
    candidatos = [
        {"provider": "routed-provider", "modelo": "general-7b", "perfil": PerfilCapacidadeModelo(categoria="general", tamanho_parametros_b=7.0, contexto=32768)},
        {"provider": "routed-provider", "modelo": "coder-7b", "perfil": PerfilCapacidadeModelo(categoria="coding", tamanho_parametros_b=7.0, contexto=32768, adequado_coding=True)},
    ]
    fallback = _SequenceProvider(["nao deve ser usado"])
    orquestrador = Orquestrador(rotador=_Rotador(fallback), provider=fallback, provider_manager=manager, roteador_inteligente=roteador, candidatos_roteamento=candidatos, hardware=hardware, planejador=lambda _: [{"id": "t1", "descricao": "escrever codigo"}])
...
def test_orquestrador_alimenta_metricas_do_provider_manager_sem_duplicar_execucao(tmp_path) -> None:
    ...
    provider = _RoutedProvider("coder-7b", tokens=True)
    manager.registrar("routed-provider", lambda **_: provider)
    orquestrador = Orquestrador(rotador=_Rotador(provider), provider=provider, provider_manager=manager, planejador=lambda _: [{"id": "t1", "descricao": "gerar resultado"}])
    resultado = orquestrador.executar("concluir tarefa")
    ...
    assert trace["metadata"]["token_usage"] == {"prompt_tokens": 9, "completion_tokens": 6, "total_tokens": 15}
