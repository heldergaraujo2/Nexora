...
class ProviderComTokens(Provider):
    ...
    def generate(self, prompt, **kwargs):
        return GenerationResult(text="ok", usage={"prompt_tokens": 7, "completion_tokens": 5, "total_tokens": 12})
...
class ProviderPriced(Provider):
    def __init__(self):
        self._modelo = "modelo-teste"
        super().__init__(name="priced", capabilities=ProviderCapability())
    ...
    def generate(self, prompt, **kwargs):
        return GenerationResult(text="ok", usage={"prompt_tokens": 1_000_000, "completion_tokens": 2_000_000, "total_tokens": 3_000_000})
...
def test_manager_executar_instancia_registra_sem_criar_outra_instancia():
    ...
    assert manager.executar_instancia("saudavel", provider, "oi").text == "ok"
...
def test_manager_calcula_custo_somente_com_preco_e_tokens_reais():
    pricing = PricingRegistry([PricingEntry("priced", "modelo-teste", "USD", 0.15, 0.60, "teste-v1", "2026-09-12", "https://example.invalid")])
    manager = ProviderManager(pricing_registry=pricing)
    manager.registrar("priced", ProviderPriced)
    resultado = manager.executar("priced", "oi")
    assert manager.calcular_custo(ProviderPriced(), resultado) == pytest.approx(1.35)
    stats = manager.estatisticas_provider("priced")
    assert stats["custo_total"] == pytest.approx(1.35)
    assert stats["geracoes_com_custo"] == 1


def test_manager_nao_estima_custo_sem_preco():
    manager = ProviderManager()
    manager.registrar("tokens", ProviderComTokens)
    resultado = manager.executar("tokens", "oi")
    assert manager.calcular_custo(ProviderComTokens(), resultado) is None
    assert manager.estatisticas_provider("tokens")["custo_total"] == 0.0
    assert manager.estatisticas_provider("tokens")["geracoes_com_custo"] == 0
