...
class Orquestrador:
    ...
    def _executar_tarefa(self, tarefa, provider, *, trace: ExecutionTrace | None = None):
        ...
        if ferramenta is not None:
            ...
            argumentos = {"ferramenta": ferramenta, "parametros": tarefa.get("parametros", {}), "solicitante": self.agent_id, "contexto": {"objetivo_id": tarefa.get("objetivo_id"), "tarefa_id": tarefa.get("id"), "descricao": descricao}}
            ...
            resultado = self.ferramentas.executar(ferramenta, argumentos["parametros"], solicitante=argumentos["solicitante"], contexto=argumentos["contexto"], execucao_id=tarefa.get("execucao_id"), idempotencia_chave=idempotencia_chave)
    ...
    def _executar_runtime(self, tarefa_dict, provider, *, routing_candidates: list[CandidatoRoteamento] | None = None):
        ...
        def executar(_prompt):
            return self._executar_tarefa(tarefa_dict, provider, trace=trace)
    ...
    def executar(self, objetivo_texto, alias=None):
        ...
        resultado = {"objetivo_id": objetivo.id, "texto": objetivo.texto, "sucesso": ok_geral, "saida": etapas[-1]["saida"] if etapas else "", "etapas": etapas, "metricas": objetivo.metricas, "historico": self.historico}
